...
var initSocket = exports.initSocket = function(options, onopen, onlistening, onmessage, onerror, onclose) {
  ...
  socket.onopen = function() {
    listening = false;
    var hardsocketstop = function(msg, data){
      //console.log( msg, data );
      console.log('MICROPHONE: close.');
      socket.send(JSON.stringify({action:'stop'}));
      socket.close();
    };
    var expected_msg = PubSub.subscribe( 'hardsocketstop', hardsocketstop );
  ...
  socket.onclose = function(evt) {
    ...
    PubSub.unsubscribe( 'hardsocketstop', 'stop' );

    PubSub.publish('newquestion', $('#resultsText').val());
